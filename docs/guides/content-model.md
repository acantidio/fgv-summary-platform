...
Files in `content/enriched/` use four callout types. These are generated by `enrich.js` running Claude Opus — do not write them by hand in `content/*.md`. When `render.js` renders the subject page, it passes these callouts to Claude Opus as semantic signals — each type gets a distinct visual treatment (hero block, alert banner, definition card, question card).
